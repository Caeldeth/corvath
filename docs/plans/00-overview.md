...
| Settings          | **A page, with Appearance and About only** (HTOO-408, 2026-08-15). The theme picker moved out of `TitleBar.tsx` — a permanent control in the window chrome for something set once. The roster stops there deliberately; the three candidates that were left open (default deck, default spread, default draw mode) are parked in [00a-backlog.md](00a-backlog.md) with the trigger that would justify them. **Declined outright:** a global reversals override (contradicts the per-deck `supportsReversed` beside it), a launch-time update-check toggle (stays unconditional, and settling it corvath-first would pre-empt HTOO-65's house-wide question), and a confirm-before-delete toggle (a preference to disable a guard on unrecoverable data is a preference to lose data). "Open the data folder" is a **button on the About card**, not a setting of its own. There is no Report-an-issue button: corvath is a personal repo with no report module and no triage behind it, so the `github.com/hybrasyl` link is the honest version. |
