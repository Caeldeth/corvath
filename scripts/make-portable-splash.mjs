 ...
 *
 * Ported from `mabon/scripts/make-portable-splash.mjs` (via dagda and taliesin).
 * The splash DESIGN is shared across the skeleton, so the gradient, border and
 * palette came across untouched; corvath's own metrics differ and are marked
 * "corvath:" below. Treat a large diff against mabon's copy as a signal that the
 * splash has drifted, not that the script needed porting.
 *
 ...
 * squaring them off is barely visible. The spinner cannot animate either.
 *
...
const LOGO_PX = 84 // corvath: .glyph is 84px (mabon's .logo is 88)
const LOGO_RADIUS = 14 // corvath: .glyph has border-radius 14px; mabon's logo has none
const TITLE_PT = 24 // corvath: .title font-size 24px (mabon 22)
const TITLE_KERN = 6 // corvath: .title letter-spacing 6px (mabon 2)
const SUBTITLE_PT = 12 // .subtitle font-size
const SUBTITLE_KERN = 1 // corvath: .subtitle letter-spacing 1px (mabon 0.5)
const SUBTITLE_MARGIN = -6 // corvath: .subtitle margin-top -6px (mabon -8)
const SPINNER_D = 22 // .spinner width/height
const SPINNER_STROKE = 2 // .spinner border-width
const GAP = 14 // corvath: .card gap 14px (mabon 16)

const WORDMARK = 'CORVATH'
// corvath: the subtitle is the app's descriptor, NOT "Loading…". The two screens
// appear back to back, so the BMP must say exactly what splash.html says.
const SUBTITLE = 'Tarot'

// ── Layout, derived from the card's flex column ───────────────────────────────
// justify-content: center, with .subtitle's negative margin and .spinner's +4px.
...
const contentH =
  LOGO_PX + GAP + titleH + GAP + (subtitleH + SUBTITLE_MARGIN) + GAP + (SPINNER_D + 4)
...
// starts. verifyAgainstBrowser() enforces that this stays within tolerance.
const TOP_CALIBRATION = -2
const top = Math.round((H - contentH) / 2) + TOP_CALIBRATION

const logoTop = top
const titleTop = logoTop + LOGO_PX + GAP
const subtitleTop = titleTop + titleH + GAP + SUBTITLE_MARGIN
...
const tmp = mkdtempSync(join(tmpdir(), 'corvath-splash-'))
try {
  ...
  // 4. Logo at its CSS size, with .glyph's 14px border-radius applied. Corvath's
  //    master is an opaque square tile — the rounding lives in CSS, so without
  //    this the frozen frame shows hard corners the real splash never has.
  const logo = join(tmp, 'logo.png')
  const logoMask = join(tmp, 'logo-mask.png')
  magick([
    '-size',
    `${LOGO_PX}x${LOGO_PX}`,
    'xc:black',
    ...
    'white',
    '-draw',
    `roundrectangle 0,0 ${LOGO_PX - 1},${LOGO_PX - 1} ${LOGO_RADIUS},${LOGO_RADIUS}`,
    logoMask
  ])
  magick([
    master,
